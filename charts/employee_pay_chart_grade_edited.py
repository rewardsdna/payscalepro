...
# Load pay range data
pay_ranges_df = pd.read_csv("data_csv/grade_edited.csv")
...
def create_chart(df, pay_ranges_df):
  ...
  if 'Base Pay' in df.columns:
      df["Base Pay"] = df["Base Pay"].fillna("0").astype(str).str.replace(",", "").astype(float)
  else:
  ...
  # Convert pay range columns to numeric, handling any non-numeric data
  for column in ['Range Min', 'Range Mid', 'Range Max']:
      pay_ranges_df[column] = pay_ranges_df[column].fillna("0").astype(str).str.replace(",", "").astype(float)
